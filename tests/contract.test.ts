...
describe('postman-api-onboarding-action composite contract', () => {
  ...
  describe('Phase 2: Step Wiring', () => {
    ...
    it('uses pinned bootstrap, repo-sync, junit-runner, junit-uploader, and insights actions', () => {
      ...
      expect(bootstrapStep?.uses).toBe('postman-cs/postman-bootstrap-action@v2.21.8');
      expect(repoSyncStep?.uses).toBe('postman-cs/postman-repo-sync-action@v2.10.8');
    ...
    it('forwards local spec-path and spec-files-json to bootstrap only when spec-url is empty', () => {
      ...
      ).toBe('postman-cs/postman-repo-sync-action@v2.10.8');
      expect(
