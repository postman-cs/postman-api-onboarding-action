...
describe('advance-pins workflow', () => {
  ...
  it('documents the branch-push and pull-request-only release path', () => {
    ...
    expect(normalizedPolicy).toContain('promoted rolling major alias');
    expect(normalizedPolicy).toContain('release-manifest.json');
    expect(normalizedPolicy).toContain('pushes only a topic branch');
    expect(normalizedPolicy).toContain('opens a pull request');
    expect(normalizedPolicy).toContain('never writes `main` directly');
    expect(normalizedPolicy).toContain('Majors never advance automatically');
    expect(normalizedPolicy).not.toMatch(/HEAD:main/);
  });
});
